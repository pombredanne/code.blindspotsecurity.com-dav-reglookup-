...
char* stack2Path(void_stack* nk_stack)
{
  ...
  while((cur = void_stack_iterator_next(iter)) != NULL)
  {
    ...
    {
      grow_amt = (unsigned int)(buf_len/2);
...
int main(int argc, char** argv)
{
  ...
  REGF_NK_REC* root;
  int retr_path_ret;
  char* path = "/ControlSet002/Services/Eventlog/";
  ...
  {
    path_stack = path2Stack(path);
    if(void_stack_size(path_stack) < 1)
      printKeyTree(f, nk_stack, "");
    else
    {
      retr_path_ret = retrievePath(f, nk_stack, 
				   path_stack);
      if(retr_path_ret == 1)
	printf("WARNING: specified path not found.\n");
